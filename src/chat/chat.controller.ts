...
register(
  ...
  @UseGuards(WsguardGuard)
  async sendMessage(@ConnectedSocket() client : Partial<AuthenticatedSocket>, @MessageBody() payload:any){
       ...
       if(senderId){
        await this.chatService.createMessage({message:payload.message},senderId,receiverId)
      }
