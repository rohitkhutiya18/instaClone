...
export class PostController {
  ...
  @UseGuards(JwtGaurd)
  @Delete('delete-post')
  deletePost(@Body() body: { postId: string }, @Req() req: Request) {
    const user = req.user as any;
    return this.postService.removePost(body.postId, user.id);
  }
  ...
  @UseGuards(JwtGaurd)
  removePostImage(
    @Body() body: { postId: string; imageId: string },
    @Req() req: Request
  ) {
    const user = req.user as any;
  ...
  updatePostImage(
    @Body() body: { postId: string; oldImgId: string },
    @Req() req: Request,
    @UploadedFile() file: Express.Multer.File,
  ) {
    const user = req.user as any;
    return this.postService.updateImage(
      body.postId,
      user.id,
      body.oldImgId,
      file.path,
